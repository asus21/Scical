...
public class CartClassifier {
	private double gini_threshold;
	private double gini_dec_threshold;
	private int min_sample_split;
	private Node treeNode;
	public static void main(String[] args) throws Exception {
	Table aTable=JCSV.read_csv("iris");
	Matrix xMatrix=aTable.data().getColRange(0,4);
	Matrix yMatrix=aTable.data().getCol(4);
	CartClassifier aCartClassifier=new CartClassifier();
	...
	public CartClassifier()
	{
		gini_threshold=0.01;
		gini_dec_threshold=0;
		min_sample_split=2;
	}
	public CartClassifier(double gini_threshold,double gini_dec_threshold,int min_sample_split)
	{
		this.gini_threshold=gini_threshold;
		this.gini_dec_threshold=gini_dec_threshold;
		this.min_sample_split=min_sample_split;
	}
	private double Gini(Matrix Y)
	{
	...
	private double bestPoint(Matrix x,Matrix y,double value)
	{
		...
		double res=Gini(y.getRowChoice(posleft))*posleft.length+Gini(y.getRowChoice(posright))*posright.length;
		return res/y.size();
	}
	...
	{		
	    ...
	    if(Gini(y)-temp[index].value<gini_dec_threshold)
	    {
	...
	{ 
	   ...
	   node.value=(double) linalg.argmax(elementcount)[0];
	   if(Y.size()<min_sample_split)
	   {
		   return node;
	   }
	  
	   else if(Gini(Y)<gini_threshold)
		   return node;
	   else 
	   {
